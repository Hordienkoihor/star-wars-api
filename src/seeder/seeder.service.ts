    ...
    async runComprehensiveSeed() {
        try {
            ...
            for (const s of rawSpecies) {
                await this.speciesService.add({...s, people: [], films: []});
            }
            ...
            for (const f of rawFilms) {
                ...
                if (dbFilm) {
                    await this.filmsService.update(dbFilm.id, {
                        ...f,
                        characters: this.extractIds(f.characters),
                        species: this.extractIds(f.species),
                        vehicles: this.extractIds(f.vehicles),
                        starships: this.extractIds(f.starships),
                        planets: this.extractIds(f.planets),
                    });
                }
            }
            ...
            for (const p of rawPeople) {
                ...
                if (dbPerson) {
                    await this.peopleService.update(dbPerson.id, {
                        ...
                        films: this.extractIds(p.films),
                        species: this.extractIds(p.species),
                        vehicles: this.extractIds(p.vehicles),
                        starships: this.extractIds(p.starships),
                    });
                }
            }
